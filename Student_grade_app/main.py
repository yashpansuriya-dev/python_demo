...
async def ensure_logged_in(page, email: str, password: str, fingerprint: dict, target_url: str) -> None:
    ...
    # Re-navigate to the intended target (e.g. /people/ page)
    if target_url and target_url not in post_login_url:
...
async def login_if_needed(page, email: str, password: str, fingerprint: dict) -> None:
    ...
    await click_first_visible(page, ["button[type='submit']", "button:has-text('Sign in')"], timeout_ms=5000)
...
_NOISE_WORDS = re.compile(
    r"\b(connect|message|follow|invite|verified|status is offline)\b",
    re.IGNORECASE,
)
